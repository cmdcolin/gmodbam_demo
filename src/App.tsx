...
function App() {
  ...
  useEffect(() => {
    ;(async () => {
      let bam
      if (files) {
        let bamIdx = files[0].name.endsWith('bam') || files[0].name.endsWith('cram') ? 0 : 1
  ...
  return (
    <div>
      ...
      >
        ...
        <input id="loc" type="text" value={loc} onChange={(event) => setLoc(event.target.value)} />
      ...
      <Canvas>
        <ambientLight />
        {[-45, -35, -25, -15, -5, 5, 15, 25, 35, 45].map((pos) => (
          <Box key={pos} position={[pos, 0, 0]} />
        ))}
      </Canvas>
    </div>
  )
